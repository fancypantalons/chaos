public class SonicWriter
{
  ...
  public int[] readAll() { return output_buf; }
  public int compressedSize() { return output_pos; }
  ...
  int seg_cmp(int start, int length, int offset)
  {
    ...
    for (i = 0; i < length; i++)
      if (buffer[start + i] != buffer[start + i + offset]) return -1;
  ...
  void compress()
  {
	...
	while ((pos + length) < buf_size)
	  {
	    if (length >= 256) break;
	    if (buffer[pos] != buffer[pos + length]) break;
	    length++;
	  }
		...
		while ((pos + length) < buf_size)
		  {
		    ...
		    if ((pos + length + 1) > buf_size) break;
		    if ((length + 1) > 256) break;
